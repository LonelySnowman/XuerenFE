...
1. JS代码分为同步任务和异步任务。
2. 同步任务会进入主线程，异步任务会进入 Event Table（事件表），当事件表中的异步任务完成后会在 Event Queue（事件队列）中注册回调函数。
3. 主线程任务全部完成后，才会完成 Event Queue 中的任务。
4. js解析器会不断地重复检查主线程执行栈是否为空，然后重复第3步，这就是 Event Loop（事件循环）。
...
**常见垃圾回收策略**：
// TODO: LJQFALG 垃圾回收机制必看
[JS垃圾回收机制【图文解析】在JavaScript中，gc代表垃圾回收（Garbage Collection）。垃圾回收 - 掘金 (juejin.cn)](https://juejin.cn/post/7274146202496090170?searchId=2024090215561928DE211FF950248D30AB#heading-3)
...
var let const function 直接声明无关键词 (共五种方法)
